...
//===----------------------------------------------------------------------===//
//
// This file contains definitions of most TensorView operations.
//
//===----------------------------------------------------------------------===//
...
//===----------------------------------------------------------------------===//
//
// The majority of the TensorView API is implemented in terms of 'ops' that are
...
//     result = #tfop("Const", dtype: Float.self, value$TensorView: 4.0)
//
...
public extension TensorView where Scalar : Numeric {
    ...
    init(_ other: TensorView<Bool>, using stream: DeviceStream? = nil) {
        if let _ = stream {
            self = TensorView()
        } else {
            self = TensorView(other)
        }
    }
}
